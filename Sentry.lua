...
function Sentry:GetSentryDamageUpgrade()

	if GetHasTech(self, kTechId.SentryDamageTech, true) then
		return kSentryUpgradedDamage
	else 
		return 0
	end
end	

function Sentry:AdjustMaxArmor()

	if GetHasTech(self, kTechId.SentryArmorTech, true) then
		self.maxArmor = 200
	end
end	
    ...
    function Sentry:FireBullets()
        ...
        for bullet = 1, Sentry.kBulletsPerSalvo do
            ...
            if trace.fraction < 1 then
                ...
                local damage = kSentryDamage + self:GetSentryDamageUpgrade()
...
function GetSentryBatteryInRoom(origin)
    ...
    if locationName then
        ...
        local batteries = Shared.GetEntitiesWithClassname("Sentry")
...
function GetRoomHasNoSentryBattery(techId, origin, normal, commander)
    ...
    if locationName then
        ...
        for index, sentryBattery in ientitylist(Shared.GetEntitiesWithClassname("Sentry")) do
            
            if sentryBattery:GetLocationName() == locationName > 3 then
                validRoom = false
                break
            end
