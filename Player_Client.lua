...
function PlayerUI_GetIsDead()
    ...
    if player then
        ...
        if player.concedeSequenceActive == true then
            return false
        end
...
function PlayerUI_GetPlayerShieldpackFuel()
...
function Player:GetDrawWorld(isLocal)
    return not self:GetIsLocalPlayer() or self:GetIsThirdPerson() or (((self.concedeSequence or self.countingDown) and not Shared.GetCheatsEnabled()) and self:GetTeamNumber() ~= kNeutralTeamType)
end
...
function Player:GetCameraViewCoordsOverride(cameraCoords)
    ...
    -- concede sequence
    if self.concedeSequenceActive == true then
        local theta = self.concedeStartAngle + ( self.concedeAngularVelocity * ( Shared.GetTime() - self.concedeStartTime ) )
        local origin = self.concedeTargetPoint
        local distance = kConcedeIdealDistance[self.concedeSettingsIndex]
        local pos = Vector( distance * math.sin(theta), kConcedeIdealHeightOffset[self.concedeSettingsIndex], distance * math.cos(theta) ) + origin
        --Log("Animating camera at: %s", pos)
        cameraCoords = Coords.GetLookAt(pos,origin, Vector(0,1,0))
        return cameraCoords
    end
