...
Gorge.kSlideCoolDown = 1
Gorge.kBellySlideControl = 18
...
function Gorge:GetCelerityArmorReduction()
    return kGorgeCelerityArmorReduction
end
...
function Gorge:GetMovementSpecialCooldown()
    ...
    local slideDelay = self.kSlideCoolDown
...
function Gorge:GetArmorCombinedAmount()
    return kGorgeCombinedArmor 
end
...
function Gorge:UpdateGorgeSliding(input)
    ...
    if slidingDesired and not self.sliding and self.timeSlideEnd + self.kSlideCoolDown < Shared.GetTime() and self:GetIsOnGround() and self:GetEnergy() >= kBellySlideCost then
...
function Gorge:GetMaxSpeed(possible)
    return self.kMaxGroundSpeed * self:GetSlowSpeedModifier()
end
...
function Gorge:ModifyCelerityBonus(celerityBonus)
...
function Gorge:PostUpdateMove(input, runningPrediction)
    ...
    if self:GetIsBellySliding() and self:GetIsOnGround() then
		...
		self.kMaxSlidingSpeed = self.kMaxSlidingSpeed - self:GetCelerityArmorScalar()
