...
function Marine:OnTakeDamage(damage, attacker, doer, point)
    ...
    if doer then
        ...
        if (doer:isa("Gore") or doer:isa("Shockwave")) and not self:GetIsVortexed() and not attacker.isHallucination then
    ...
    if hostStructure then
        ...
        if mapName then
            ...
            if techId == kTechId.Jetpack then
                ...
                self:AddResources(-GetCostForTech(techId))
                self:GiveJetpack()            
			elseif techId == kTechId.Shieldpack then
...
function Marine:GiveShieldpack()
    ...
    local shieldpackMarine = self:Replace(ShieldpackMarine.kMapName, self:GetTeamNumber(), true, Vector(self:GetOrigin()))
    
    shieldpackMarine:SetActiveWeapon(activeWeaponMapName)
    shieldpackMarine:SetHealth(health)
...
function Marine:GiveJetpack()
    ...
    local shieldpackMarine = self:Replace(JetpackMarine.kMapName, self:GetTeamNumber(), true, Vector(self:GetOrigin()))
